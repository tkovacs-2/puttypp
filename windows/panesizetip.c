...
void size_tip_update(SizeTip *size_tip, int width, int height, const RECT *rect) {
    ...
    if (changed || size_tip->text[0] == L'\0') {
        ...
        HGDIOBJ oldfont = SelectObject(hdc, size_tip_font);
        GetTextExtentPoint32W(hdc, size_tip->text, wcslen(size_tip->text), &size_tip->size);
        SelectObject(hdc, oldfont);
        DeleteDC(hdc);
        size_tip->size.cx += SIZE_TIP_PADDING * 2;
        size_tip->size.cy += SIZE_TIP_PADDING * 2;
    }

    if (size_tip->size.cx > rect->right - rect->left ||
        size_tip->size.cy > rect->bottom - rect->top) {
        if (size_tip->hwnd) {
            ShowWindow(size_tip->hwnd, SW_HIDE);
        }
        return;
    }

    if (size_tip->hwnd) {
        SetWindowPos(size_tip->hwnd, NULL, 0, 0, size_tip->size.cx, size_tip->size.cy,
            SWP_NOZORDER | SWP_NOMOVE | SWP_NOACTIVATE | SWP_NOREDRAW | SWP_SHOWWINDOW);
        InvalidateRect(size_tip->hwnd, NULL, FALSE);
    } else {
        POINT screen = {rect->left, rect->top};
        ClientToScreen(frame_hwnd, &screen);
        size_tip->hwnd = CreateWindowExW(
            0, SIZE_TIP_CLASS_NAME, NULL, WS_POPUP,
            screen.x, screen.y, size_tip->size.cx, size_tip->size.cy,
            frame_hwnd, NULL, hinst, size_tip);
        SetWindowPos(size_tip->hwnd, HWND_TOP, 0, 0, 0, 0,
...
HDWP size_tip_move(SizeTip *size_tip, const POINT *point, HDWP hdwp) {
    if (size_tip->hwnd) {
        POINT screen = *point;
        ClientToScreen(frame_hwnd, &screen);
        hdwp = DeferWindowPos(hdwp, size_tip->hwnd, NULL, screen.x, screen.y, 0, 0,
