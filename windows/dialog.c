...
static char *getevent(eventlog_stuff *es, int i)
{
    if (i < es->ninitial)
        return es->events_initial[i];
    if ((i -= es->ninitial) < es->ncircular)
        return es->events_circular[(es->circular_first + i) % LOGEVENT_CIRCULAR_MAX];
    return NULL;
}
...
void reseteventlog(eventlog_stuff *es) {
    ...
    SendDlgItemMessage(logbox, IDN_LIST, LB_RESETCONTENT, 0, 0);
    for (int i = 0; i < es->ninitial; i++)
        SendDlgItemMessage(logbox, IDN_LIST, LB_ADDSTRING,
                           0, (LPARAM) es->events_initial[i]);
    for (int i = 0; i < es->ncircular; i++)
        SendDlgItemMessage(logbox, IDN_LIST, LB_ADDSTRING,
          ...
          case IDN_COPY:
            if (HIWORD(wParam) == BN_CLICKED ||
                ...
                if (selitems) {
                    ...
                    eventlog_stuff *es = (eventlog_stuff *)GetWindowLongPtr(hwnd, GWL_USERDATA);

                    size = 0;
                    for (i = 0; i < count; i++)
                        size +=
                            strlen(getevent(es, selitems[i])) + sizeof(sel_nl);

                    clipdata = snewn(size, char);
                    if (clipdata) {
                        char *p = clipdata;
                        for (i = 0; i < count; i++) {
                            char *q = getevent(es, selitems[i]);
                            int qlen = strlen(q);
                            memcpy(p, q, qlen);
                            p += qlen;
                    ...
                    for (i = 0; i < (es->ninitial + es->ncircular); i++)
                                    ...
                                    WPARAM wParam, LPARAM lParam)
{
    switch (msg) {
      case WM_INITDIALOG: {
        SetWindowText(hwnd, "PuTTY Licence");
                                  ...
                                  WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg) {
      case WM_INITDIALOG: {
        ...
        sfree(text);
        SetDlgItemText(hwnd, IDA_LICENCE, "PuTTY &Licence");
        return 1;
      }
      case WM_COMMAND:
        switch (LOWORD(wParam)) {
          ...
          case IDA_WEB:
            ...
            ShellExecute(hwnd, "open",
                         "https://github.com/tkovacs-2/puttypp/",
                         0, 0, SW_SHOWDEFAULT);
            return 0;
        }
        return 0;
...
struct sessionsaver_data {
    ...
    bool midsession;
    const char *savedsession;
};

static const char *get_session_name(struct controlbox *ctrlbox)
{
    struct controlset *s = ctrl_getset(ctrlbox, "Session", "savedsessions", NULL);
    for (int i=0; i<s->ncontrols; i++) {
        dlgcontrol *c = s->ctrls[i];
        if (c->type == CTRL_EDITBOX) {
            struct sessionsaver_data *ssd = (struct sessionsaver_data *)c->context.p;
            const char *session_name = ssd->savedsession;
            ssd->savedsession = NULL;
            return session_name;
        }
    }
    return NULL;
...
static void set_session_name(const char *session_name, struct controlbox *ctrlbox)
{
    struct controlset *s = ctrl_getset(ctrlbox, "Session", "savedsessions", NULL);
    for (int i=0; i<s->ncontrols; i++) {
        dlgcontrol *c = s->ctrls[i];
...
bool do_reconfig(HWND parent, Conf *conf, const char **session_name, int protcfginfo)
{
    ...
    set_session_name(*session_name, pds->ctrlbox);
    ret = ShinyDialogBox(hinst, MAKEINTRESOURCE(IDD_MAINBOX), "PuTTYConfigBox",
                         parent, GenericMainDlgProc, pds);
    if (ret) {
        *session_name = get_session_name(pds->ctrlbox);
    }
