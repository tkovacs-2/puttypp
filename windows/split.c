...
static void move_splitter_move(Split *split) {
    int x, y;
    if (split->type == SPLIT_TYPE_VERTICAL) {
        x = split->first->planned_rect.right;
        y = split->planned_rect.top;
    } else {
        x = split->planned_rect.left;
        y = split->first->planned_rect.bottom;
    }
...
void split_update_sizetips(Split *split) {
    Pane *pane = split_get_pane(split);
    if (pane) {
        pane_update_sizetip(pane, &split->planned_rect, (const POINT *)&split->rect);
    } else {
...
void split_move_sizetips(Split *split) {
    Pane *pane = split_get_pane(split);
    if (pane) {
        pane_move_sizetip(pane, (const POINT *)&split->rect);
    } else {
        split_move_sizetips(split_get_first(split));
        split_move_sizetips(split_get_second(split));
    }
}
