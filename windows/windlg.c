...
static const char *get_session_name()
{
    ...
    for (int i=0; i<s->ncontrols; i++) {
        ...
        if (c->generic.type == CTRL_EDITBOX) {
            struct sessionsaver_data *ssd = (struct sessionsaver_data *)c->generic.context.p;
...
bool do_config(HWND parent, Conf *conf, const char **session_name)
{
    ...
    ret =
        SaneDialogBox(hinst, MAKEINTRESOURCE(IDD_MAINBOX), parent,
                  GenericMainDlgProc);
    if (ret) {
        const char *t = get_session_name();
        if (*t) {
            *session_name = t;
        } else {
            sfree((char *)t);
...
bool do_reconfig(HWND parent, Conf *conf, const char **session_name, int protcfginfo)
{
    ...
    ret = SaneDialogBox(hinst, MAKEINTRESOURCE(IDD_MAINBOX), parent,
                  GenericMainDlgProc);
    if (ret) {
    ...
    char *keystr, const char *keydisp, char **fingerprints)
{
    ...
    if (ret == 0)                      /* success - key matched OK */
        ...
        int mbret = DialogBoxParam(
            hinst, MAKEINTRESOURCE(dlgid), hwnd,
